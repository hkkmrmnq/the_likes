...
6. 6.1. Each person has an internal hierarchy of values, which influences his emotions, thoughts, language, decisions and behavior, etc. And / or:
...
6. 6.1. У каждого человека есть внутренняя иерархия ценностей, которая оказывает влияние на его эмоции, мысли, язык, решения и поведение и т.д. И / или:
...
API documentation file - 'documentation.json' - in root folder. To read upload to https://editor.swagger.io/
...
Run celery worker (--pool=solo if on Windows):

```bash
uv run celery -A src.tasks worker --pool=solo --loglevel=info
```
