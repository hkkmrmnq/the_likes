...
prepare_recommendations_raw_str: list[str] = [
    """
DROP FUNCTION IF EXISTS public.array_jaccard_similarity CASCADE;
    """,
    """
DROP FUNCTION IF EXISTS public.compare_moral_profiles;
    """,
    """
DROP FUNCTION IF EXISTS public.search_status_sort_priority;
    """,
    """
...
public.array_jaccard_similarity(arr1 INTEGER[], arr2 INTEGER[])
RETURNS FLOAT AS $$
DECLARE
    intersection INT;
    union_size INT;
...
BEGIN
    ...
    RETURN
        ...
        ELSE public.array_jaccard_similarity(best1, best2) * 0.2 END +
        CASE WHEN worst1 = worst2 THEN 0.35
        ELSE public.array_jaccard_similarity(worst1, worst2) * 0.2 END +
        public.array_jaccard_similarity(good1, good2) * 0.1 +
        public.array_jaccard_similarity(bad1, bad2) * 0.1 +
        public.array_jaccard_similarity(neutral1, neutral2) * 0.1;
END;
$$ LANGUAGE plpgsql IMMUTABLE;
    """,
    f"""
...
ARRAY(
    ...
    AND pv.user_order
    <= {CNST.PERSONAL_VALUE_MAX_ORDER - CNST.NUMBER_OF_WORST_UVS}
    ORDER BY pv.user_order
) AS bad_uv_ids,
...
WHERE EXISTS (SELECT 1 FROM personalvalues WHERE user_id = p.user_id);
    """,
    """
CREATE UNIQUE INDEX IF NOT EXISTS idx_moral_profiles_user_id_unique
ON moral_profiles (user_id);
    """,
    """
CREATE INDEX IF NOT EXISTS idx_moral_profiles_best_uv_ids
ON moral_profiles USING gin (best_uv_ids);
    """,
    """
CREATE INDEX IF NOT EXISTS idx_moral_profiles_good_uv_ids
ON moral_profiles USING gin (good_uv_ids);
    """,
    """
CREATE INDEX IF NOT EXISTS idx_moral_profiles_neutral_uv_ids
ON moral_profiles USING gin (neutral_uv_ids);
    """,
    """
CREATE INDEX IF NOT EXISTS idx_moral_profiles_bad_uv_ids
ON moral_profiles USING gin (bad_uv_ids);
    """,
    """
CREATE INDEX IF NOT EXISTS idx_moral_profiles_worst_uv_ids
...
allowed_for_search AS (
    ...
    WHERE search_allowed_status IN (
        '{SearchAllowedStatus.OK.value}',   --'ok',
        '{SearchAllowedStatus.SUSPENDED.value}'  --'suspended'
    )
),
...
profile_pairs AS (
    SELECT
        ...
        WHERE
            ...
            AND p1.best_uv_ids && p2.best_uv_ids
            AND public.array_jaccard_similarity(
...
ON recommendations USING GIN (user_ids);
    """,
    """
CREATE UNIQUE INDEX IF NOT EXISTS idx_recommendations__user_ids
